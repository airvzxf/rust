...
// TODO: The crossbeam package for multi-thread is not implemented.
fn main() {
    ...
    let threads: usize = 8;
    let band_rows: usize = bounds.1 / threads + 1;
            ...
            for _ in 0..threads {
                scope.spawn(|_| loop {
                    match {
                        ...
                        Some((i, band)) => {
                            let top: usize = band_rows * i;
                            let height: usize = band.len() / bounds.0;
                            let band_bounds: (usize, usize) = (bounds.0, height);
                            let band_upper_left: Complex<f64> =
                                pixel_to_point(bounds, (0, top), upper_left, lower_right);
                            let band_lower_right: Complex<f64> = pixel_to_point(
                                bounds,
                                (bounds.0, top + height),
                                upper_left,
                                lower_right,
